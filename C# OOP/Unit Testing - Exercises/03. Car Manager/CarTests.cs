...
namespace Tests
{
    public class CarTests
    {
        ...
        public void Ctor_ThrowsException_WhenDataIsInvalid(string make, string model, double fuelCons, double fuelCapa)
        {
            Assert.Throws<ArgumentException>(() => new Car(make, model, fuelCons, fuelCapa));
        }
        [Test]
        ...
        [TestCase(0)]
        [TestCase(-20)]
        public void Refuel_ThrownExcept_WhenFUelIsZeroOrNegativ(double fuel)
        {
            Assert.Throws<ArgumentException>(() => car.Refuel(fuel));
        }
        [Test]
        public void Refuel_IncreasesFuelAmount_WhenFuelAmouIsValid()
        {
            double refAmount = this.car.FuelCapacity / 2;
            this.car.Refuel(refAmount);

            Assert.That(this.car.FuelAmount, Is.EqualTo(refAmount));
        }
        [Test]
        public void Refuel_SEtFuelAmountCapacity_WhenCapacityIsExceeded()
        {
